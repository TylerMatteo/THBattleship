...
class Game:
    ...
    def __init__(self):
        # Start the game by prompting for the players' names,
        # drawing their empty boards, and then placing their ships
    ...
    def start_turns(self):
        ...
        while True:
            input("{}, it's your turn. Press enter to continue."
                  .format(this_player.name))
            clear_screen()
            print("{}'s board:".format(other_player.name))
            this_player.draw_opponent_board()
            this_player.draw_board()

            # Prompt the user for coordinates to fire at
            # until they enter ones that exist in the other player's
            # board and that they haven't already fired at.
            while True:
                print("{}, what coordinate would you like to fire at?"
                      .format(this_player.name))
                column = input('Column: ').strip().upper()
                row = input('Row: ')

                if (column not in [chr(i+65) for i in range(0, BOARD_SIZE)]):
                        print("Input not within choices of columns."
                              "Please try again.")
                        continue
            ...
            # See if the given coordinate exists in the any of the
            # other player's ship's coordinates
            for ship in other_player.fleet:
                if (column, row) in ship.coords.keys():
                    ...
                    # When that is the case, change it's pieces to sunk,
                    # set it to sunk, and update the
                    # coordinates in shots_taken
                    if all(space == PIECES['hit']
                           for space
                           in ship.coords.values()):
                        print("You sank {}'s {}!"
                              .format(other_player.name, ship.name))
                        ship.is_sunk = True
                        for (column, row) in ship.coords.keys():
                            ship.coords[(column, row)] = PIECES['sunk']
                            this_player.shots_taken[
                                        (column, row)
                                        ] = PIECES['sunk']
                    break
            # If the player misses, notify them
            # and then prompt them to hand off before continuing
            else:
                print("Miss!")
                this_player.shots_taken[(column, row)] = PIECES['miss']
                other_player.misses[(column, row)] = PIECES['miss']

            # If all of the opponent's ships are sunk,
            # notify this player that they have won and end the game
            if all(ship.is_sunk for ship in other_player.fleet):
                clear_screen()
                print("Congratulations {}, "
                      "you sank all of {}'s ships! You win!"
                      .format(this_player.name, other_player.name))
                for player in self.player1, self.player2:
                    player.draw_board()
                break
            else:
                input("Please press enter and then hand off to {}."
